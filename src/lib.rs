...
//!  Crucially, resources includes the owned buffer.
...
//! More background on this: withoutboats' [notes on io_uring](https://without.boats/blog/io-uring/).
//!
//! ## Example
//!
//! ```
...
//! executor thread to spawned thread, and then back to executor thread once the `read()` call completes.
...
//!
//! That overhead is justified if the `read()` operation is going to to block for the disk.
//! For example, a page-cache-missing 8k random `read()` an `i4i.2xlarge` EC2 instance takes `~110us`.
//! In this case, `tokio:fs` is clearly a better choice than blocking the executor thread through sync IO.
//!
...
//!
//! The problem is: we don't know ahead of itme if it's going to be a hit or miss.
...
//!
//! Now, one could argue once can justs witch to [`tokio_uring`](https://docs.rs/tokio-uring/latest/tokio_uring/).
...
//!
//! So, the next best idea is to use `tokio` and `tokio_uring` within the same proces like so:
//! - Use `tokio` by default.
//! - From within the `tokio` executor thread,
//!   - create `tokio_uring` futures
//!   - spawn a task on the `tokio_uring` runtime to drive the future to completion
//!   - use oneshot channel to await the completion on the `tokio` executor thread
//!
...
//! ahd hence not the same threads as the `tokio` runtime's executor threads where we're `await`ing the
//! oneshot channel receiver.
//! So, delivery of the completion is a again a context switch.
//! (It's also an outside-of-runtime `wake()` for `tokio`, which allegedly is more expensive than
//!  intra-runtime `wake()`s. Make sense, but I haven't measured it in isolation).
//!
//! # Design
//!
//! Enter this crate.
...
//! * We can use Linux's new-ish `io_uring` facility to do submit kernel-buffered reads from async Rust.
...
//! * We can issue kernel-buffered read system from async Rust without risking to blocking the executor thread.
...
//! While it's quite ugly, it avoids synchronizing on the submission path with signficant performance benefits.
...
//! If the system is under heavy load, turnaround time for the *poller task* in the tokio scheduler will grow.
//! This means completion processing gets delayed, thereby transitively delaying `wake()` of our `Future`s that issues these operations.
//! It would be beneficial to priortize the *poller task* if the io_uring fd becomes EPOLLIN.
//!
...
//!      This means that the *poller task*s float freely among the executor OS threads.
//!      In a scenario where a read hits the page cache, the *poller task* will thus run on a different OS thread
//!      than where the read op was issues. So, tokio will have to do an (intra-runtime) cross-OS-thread `wake()`.
...
//!      So, we could end up with a lot of io_uring instances in the system if `block_in_place` is used a lot.
//!      It's not a problem per-se, and certainly, we made sure that it doesn't impact safety or liveness.
//!      But it's an ugly wrinkle that wouldn't be necessary if we could just fork `tokio` to integrate `epoll`'d `io_uring` more deeply.
//!
...
//! The total working set is ca 40 GiB and we made sure it was completely inside page cache during the benchmark (no read IOs to the disk during benchmark).
//!
//! We have some env-var tunables to maximize fairness.
//! Enabling them yields 1.8 million 8k random reads per second at near 100% fairness.
//! Fairness is measured by the spread of total achieved IOPS per task over the benchmark run.
//! Lower spread is better fairness.
//!
//! The bottleneck in any of above configurations is CPU time.
//! We reach 100% CPU utlization with far less than 400 concurrent tokio tasks.
//! The 400 are for stress-testing fairness.
//! Flamegraphs show > 75% of CPU time spent in the kernel, doing the actual work.
//!
//! In terms of latency jitter, the maximially fair configuration has mean latency of 204us and p99.99 latency of 793us.
//! The most performant configuration had mean of 2us, p99.99 of 19us; which implies very-long-tail outliers.
//!
...
//!
//! For comparison, `tokio::fs` will do just about ca 370k IOPS; we din't measure latency jitter.
//!
...
//! Mean latency is 5.2ms and p99.99 latency is 21ms.
//! Clearly, the bottleneck is the disk and significant here (single-threaded random read latency is `~110us`).
//! So, significant queuing is going on here.
...
//! CPU utlization is higher, but acceptable.
