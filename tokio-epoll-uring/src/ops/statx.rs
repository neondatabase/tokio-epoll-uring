...
pub use uring_common::linux_raw_sys::general::{statx, AT_STATX_SYNC_AS_STAT};
use uring_common::{
    io_fd::IoFd,
    io_uring::{self},
};
    ...
    fn make_sqe(&mut self) -> io_uring::squeue::Entry {
        ...
        match self {
            StatxOp::ByFileDescriptor { file, statxbuf } => {
                ...
                .flags(libc::AT_EMPTY_PATH | (AT_STATX_SYNC_AS_STAT as i32))
                .mask(uring_common::linux_raw_sys::general::STATX_ALL)
                .build()
            }
        }
    }
